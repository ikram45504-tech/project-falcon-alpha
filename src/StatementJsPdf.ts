...
const COLORS = {
  ...
  adjustment: "#FFF4DE", adjustmentEdge: "#D97706",
};
...
function textField(row: Obj, key: string) { return String(row[key] ?? "").trim(); }
function numberField(row: Obj, key: string) { const n = Number(row[key] ?? 0); return Number.isFinite(n) ? n : 0; }
function universalValues(row: unknown) { return object(object(row).values); }
function inStatementPeriod(date: string, data: StatementPdfData) { return date >= data.fromDate && date <= data.toDate; }
...
function drawSummary(doc: jsPDF, data: StatementPdfData, y: number) {
  ...
  const cards = [
    ...
    { title: isVendor ? "PURCHASE ACTIVITY" : "SALE ACTIVITY", value: money(data.bookingsDuringPeriod), foot: "Bookings + adjustments in period", bg: COLORS.blueCard, top: COLORS.navy },
...
function rowHeight(doc: jsPDF, cells: Cell[], columns: Column[]) {
  return Math.max(5.8, ...cells.map((cell, i) => {
...
function drawColumns(doc: jsPDF, columns: Column[], theme: Theme, y: number) {
  ...
  columns.forEach((col, i) => {
    ...
    lines.forEach((line, lineIndex) => doc.text(line, x + col.width / 2, startY + lineIndex * gap, { align: "center" }));
    x += col.width;
  });
...
function drawRow(doc: jsPDF, row: TableRow, columns: Column[], y: number, h: number, alt?: string) {
  ...
  if (bg) { fill(doc, bg); doc.rect(MARGIN, y, CONTENT_W, h, "F"); }
  if (kind === "adjustment") { fill(doc, COLORS.adjustmentEdge); doc.rect(MARGIN, y, 0.8, h, "F"); }
  let x = MARGIN;
  stroke(doc, COLORS.border);
...
function renderSection(doc: jsPDF, data: StatementPdfData, title: string, columns: Column[], rows: TableRow[], subtotal: SectionSubtotal, theme: Theme, y: number) {
...
function originalUniversalRows(meta: StatementBookingMeta, fallback: Obj[]) {
  const rows = array(parseJson(meta.statementOriginalSnapshotJson)).map(universalValues).filter((row) => Object.keys(row).length > 0);
  return rows.length ? rows : fallback;
}
function hasOriginalSnapshot(meta: StatementBookingMeta) { return Boolean(meta.statementOriginalSnapshotJson && meta.statementAdjustments.length); }
...
function adjustmentRateSummary(service: BookingServiceName, adjustment: StatementAdjustmentRecord) {
  ...
  if (service === "PACKAGE") {
    ...
    const rows = array(parsed).map(universalValues).filter((row) => Object.keys(row).length > 0);
    if (rows.length === 1) {
      const row = rows[0];
      if (service === "TICKET") return `${money(Number(row.rate || 0))} / Ticket`;
      if (service === "HOTEL") return `${sar(Number(row.rateSar || 0))} / Night${Number(row.roe || 0) > 0 ? ` @ ${number(Number(row.roe))}` : ""}`;
      if (service === "VISA") return `${sar(Number(row.visaRateSar || 0))} / Pax${Number(row.roe || 0) > 0 ? ` @ ${number(Number(row.roe))}` : ""}`;
      if (service === "TRANSPORT") return `${sar(Number(row.rateSar || 0))}${Number(row.roe || 0) > 0 ? ` @ ${number(Number(row.roe))}` : ""}`;
      if (service === "MISC") return Number(row.roe || 0) > 0 ? `SAR ${number(Number(row.rate || 0))} / Pax @ ${number(Number(row.roe))}` : `${money(Number(row.rate || 0))} / Pax`;
    }
  }
...
function adjustmentFinanceSecondary(adjustment: StatementAdjustmentRecord, accountType: Party["account_type"]) {
  const parts: string[] = [];
  if (adjustment.charge_pkr > 0) parts.push(`${accountType === "VENDOR" ? "Supplier cost" : "Fee / charge"} +${money(adjustment.charge_pkr)}`);
...
function adjustmentReason(adjustment: StatementAdjustmentRecord) {
  const parts = [adjustment.reason.trim(), adjustment.reference.trim() ? `Ref: ${adjustment.reference.trim()}` : ""].filter(Boolean);
...
function adjustmentAmountSecondary(adjustment: StatementAdjustmentRecord, data: StatementPdfData, latest: boolean) {
  if (!inStatementPeriod(adjustment.adjustment_date, data)) return `REV ${adjustment.revision_no} | Prior-period revision reference`;
...
function packageAdjustmentRow(booking: StatementBookingMeta & { ub_number: string }, adjustment: StatementAdjustmentRecord, data: StatementPdfData, latest: boolean): TableRow {
  return { kind: "adjustment", cells: [
    { text: "A", align: "center", bold: true },
    { text: shortDate(adjustment.adjustment_date), secondary: displayUb(booking.ub_number), align: "center", bold: true },
    ...
    { text: adjustmentRateSummary("PACKAGE", adjustment), secondary: adjustmentFinanceSecondary(adjustment, data.party.account_type), align: "right" },
    { text: `R${adjustment.revision_no}`, align: "center", bold: true },
    { text: money(adjustment.effective_total_pkr), secondary: adjustmentAmountSecondary(adjustment, data, latest), align: "right", bold: true },
  ] };
}
function ticketAdjustmentRow(booking: StatementBookingMeta & { ub_number: string }, adjustment: StatementAdjustmentRecord, data: StatementPdfData, latest: boolean): TableRow {
  return { kind: "adjustment", cells: [
    { text: "A", align: "center", bold: true },
    { text: shortDate(adjustment.adjustment_date), secondary: displayUb(booking.ub_number), align: "center", bold: true },
    { text: adjustmentTypeLabel(adjustment.adjustment_type), secondary: adjustmentReason(adjustment), bold: true },
    { text: safeText(adjustment.category || "Booking change"), secondary: adjustment.reference ? `Ref: ${adjustment.reference}` : `REV ${adjustment.revision_no}` },
    { text: `Revised booking`, secondary: `Base ${money(adjustment.revised_base_pkr)}` },
    { text: adjustmentRateSummary("TICKET", adjustment), secondary: adjustmentFinanceSecondary(adjustment, data.party.account_type), align: "right" },
    { text: `R${adjustment.revision_no}`, align: "center", bold: true },
    { text: money(adjustment.effective_total_pkr), secondary: adjustmentAmountSecondary(adjustment, data, latest), align: "right", bold: true },
  ] };
}
function hotelAdjustmentRow(booking: StatementBookingMeta & { ub_number: string }, adjustment: StatementAdjustmentRecord, data: StatementPdfData, latest: boolean): TableRow {
  return { kind: "adjustment", cells: [
    { text: "A", align: "center", bold: true },
    { text: shortDate(adjustment.adjustment_date), secondary: displayUb(booking.ub_number), align: "center", bold: true },
    { text: adjustmentTypeLabel(adjustment.adjustment_type), secondary: adjustmentReason(adjustment), bold: true },
    { text: safeText(adjustment.category || "Booking change"), secondary: adjustment.reference ? `Ref: ${adjustment.reference}` : "" },
    { text: `REV ${adjustment.revision_no}`, secondary: adjustment.lifecycle_status.replace(/_/g, " ") },
    { text: adjustmentRateSummary("HOTEL", adjustment), secondary: adjustmentFinanceSecondary(adjustment, data.party.account_type) },
    { text: signedMoney(adjustment.account_delta_pkr), secondary: "Account impact", align: "right" },
    { text: money(adjustment.effective_total_pkr), secondary: adjustmentAmountSecondary(adjustment, data, latest), align: "right", bold: true },
  ] };
}
function visaAdjustmentRow(booking: StatementBookingMeta & { ub_number: string }, adjustment: StatementAdjustmentRecord, data: StatementPdfData, latest: boolean): TableRow {
  return { kind: "adjustment", cells: [
    { text: "A", align: "center", bold: true },
    { text: shortDate(adjustment.adjustment_date), secondary: displayUb(booking.ub_number), align: "center", bold: true },
    { text: adjustmentTypeLabel(adjustment.adjustment_type), secondary: adjustmentReason(adjustment), bold: true },
    { text: safeText(adjustment.category || "Visa change"), secondary: adjustment.reference ? `Ref: ${adjustment.reference}` : "" },
    { text: `R${adjustment.revision_no}`, align: "center", bold: true },
    { text: adjustmentRateSummary("VISA", adjustment), secondary: adjustmentFinanceSecondary(adjustment, data.party.account_type) },
    { text: "-", align: "right" },
    { text: signedMoney(adjustment.account_delta_pkr), secondary: "Account impact", align: "right" },
    { text: money(adjustment.effective_total_pkr), secondary: adjustmentAmountSecondary(adjustment, data, latest), align: "right", bold: true },
  ] };
}
function transportAdjustmentRow(booking: StatementBookingMeta & { ub_number: string }, adjustment: StatementAdjustmentRecord, data: StatementPdfData, latest: boolean): TableRow {
  return { kind: "adjustment", cells: [
    { text: "A", align: "center", bold: true },
    { text: shortDate(adjustment.adjustment_date), secondary: displayUb(booking.ub_number), align: "center", bold: true },
    { text: adjustmentTypeLabel(adjustment.adjustment_type), secondary: adjustmentReason(adjustment), bold: true },
    { text: safeText(adjustment.category || "Transport change"), secondary: adjustment.reference ? `Ref: ${adjustment.reference}` : "" },
    { text: `REV ${adjustment.revision_no}`, secondary: adjustment.lifecycle_status.replace(/_/g, " ") },
    { text: adjustmentRateSummary("TRANSPORT", adjustment), secondary: adjustmentFinanceSecondary(adjustment, data.party.account_type) },
    { text: signedMoney(adjustment.account_delta_pkr), secondary: "Account impact", align: "right" },
    { text: money(adjustment.effective_total_pkr), secondary: adjustmentAmountSecondary(adjustment, data, latest), align: "right", bold: true },
  ] };
}
function miscAdjustmentRow(booking: StatementBookingMeta & { ub_number: string }, adjustment: StatementAdjustmentRecord, data: StatementPdfData, latest: boolean): TableRow {
  return { kind: "adjustment", cells: [
    { text: "A", align: "center", bold: true },
    { text: shortDate(adjustment.adjustment_date), secondary: displayUb(booking.ub_number), align: "center", bold: true },
    { text: adjustmentTypeLabel(adjustment.adjustment_type), secondary: adjustmentReason(adjustment), bold: true },
    { text: safeText(adjustment.category || "Service change"), secondary: adjustment.reference ? `Ref: ${adjustment.reference}` : "" },
    { text: `R${adjustment.revision_no}`, align: "center", bold: true },
    { text: adjustmentRateSummary("MISC", adjustment), secondary: adjustmentFinanceSecondary(adjustment, data.party.account_type) },
    { text: signedMoney(adjustment.account_delta_pkr), secondary: "Account impact", align: "right" },
    { text: money(adjustment.effective_total_pkr), secondary: adjustmentAmountSecondary(adjustment, data, latest), align: "right", bold: true },
  ] };
}
...
function drawReconciliation(doc: jsPDF, data: StatementPdfData, y: number) {
  ...
  const note = "Crossed booking rows are superseded reference only and are not charged again. Each booking change stays directly under its genuine UB in one adjustment row showing the actual adjustment date, revised/effective chargeable amount and period account impact. Operational/private details remain excluded.";
...
export function buildStatementPdf(data: StatementPdfData) {
  ...
  data.sections.packageBookings.forEach((booking) => {
    const fallback = booking.lines.map((line) => ({
      passenger_type: line.passenger_type, passenger_name: line.passenger_name, package_type: line.package_type,
      rate_per_person: line.rate_per_person, person_count: line.person_count, line_total_pkr: line.line_total_pkr,
    } as Obj));
    ...
    const rows = originals.map((line) => {
      ...
      return { kind: superseded ? "reference" as const : "normal" as const, strike: superseded, cells: [
        ...
        { text: textField(line, "passenger_type") || textField(line, "passengerType"), secondary: safeText(textField(line, "package_type") || textField(line, "packageType")) },
        { text: money(rate), align: "right" },
        { text: String(qty), align: "center" },
        { text: money(total), secondary: superseded ? "SUPERSEDED | REFERENCE ONLY" : undefined, align: "right", bold: true },
      ] } as TableRow;
    });
    packageRows.push(...insertAdjustmentsAfterFirst(rows, booking.statementDisplayAdjustments, (adjustment, latest) => packageAdjustmentRow(booking, adjustment, data, latest)));
  });
  y = renderSection(doc, data, "PACKAGE BOOKINGS", packageColumns, packageRows, { label: "PACKAGE STATEMENT SUBTOTAL", pkr: statementSectionPeriodTotal(data.sections.packageBookings) }, BOOKING_THEME, y);
  ...
  data.sections.ticketBookings.forEach((booking) => {
    const fallback = booking.lines.map((line) => ({ passengerType: line.passenger_type, passengerName: line.passenger_name, airlineName: line.airline_name, pnr: line.pnr, flightType: line.flight_type, ticketRoute: line.ticket_route, rate: line.rate_per_ticket, qty: line.ticket_count } as Obj));
    const originals = hasOriginalSnapshot(booking) ? originalUniversalRows(booking, fallback) : fallback;
    const superseded = booking.statementDisplayAdjustments.length > 0;
    const rows = originals.map((line) => {
      ticketSr += 1;
      const rate = Number(line.rate || 0), qty = Math.max(1, Number(line.qty || 1));
      return { kind: superseded ? "reference" as const : "normal" as const, strike: superseded, cells: [
        ...
        { text: money(rate), align: "right" },
        { text: String(qty), align: "center" },
        { text: money(rate * qty), secondary: superseded ? "SUPERSEDED | REFERENCE ONLY" : undefined, align: "right", bold: true },
      ] } as TableRow;
    });
    ticketRows.push(...insertAdjustmentsAfterFirst(rows, booking.statementDisplayAdjustments, (adjustment, latest) => ticketAdjustmentRow(booking, adjustment, data, latest)));
  });
  y = renderSection(doc, data, "TICKET BOOKINGS", ticketColumns, ticketRows, { label: "TICKET STATEMENT SUBTOTAL", pkr: statementSectionPeriodTotal(data.sections.ticketBookings) }, BOOKING_THEME, y);
  ...
  data.sections.hotelBookings.forEach((booking) => {
    const fallback = booking.lines.map((line) => ({ city: line.city, hotelName: line.hotel_name, checkIn: line.check_in, checkOut: line.check_out, nights: line.nights, roomType: line.room_type, qty: line.quantity, rateSar: line.rate_per_night_sar, roe: line.roe } as Obj));
    const originals = hasOriginalSnapshot(booking) ? originalUniversalRows(booking, fallback) : fallback;
    const superseded = booking.statementDisplayAdjustments.length > 0;
    const rows = originals.map((line, index) => {
      ...
      const rateSar = Number(line.rateSar || 0), nights = Math.max(1, Number(line.nights || 1)), qty = Math.max(1, Number(line.qty || 1)), roe = Number(line.roe || 0);
      const totalSar = rateSar * nights * qty;
      return { kind: superseded ? "reference" as const : "normal" as const, strike: superseded, cells: [
        { text: String(hotelSr), align: "center" },
        { text: shortDate(booking.transaction_date), secondary: displayUb(booking.ub_number), align: "center", bold: true },
        { text: displayName(guestName), secondary: `${safeText(line.hotelName)} - ${safeText(line.city)}`, bold: true },
        { text: `${shortDate(String(line.checkIn || ""))} to ${shortDate(String(line.checkOut || ""))}`, secondary: countLabel(nights, "Night") },
        { text: titleCase(String(line.roomType || "")), secondary: String(line.roomType) === "SHARING" ? countLabel(qty, "Bed") : countLabel(qty, "Room") },
        { text: `${sar(rateSar)} / Night`, secondary: roe > 0 ? `ROE ${number(roe)}` : "ROE Pending" },
        { text: sar(totalSar), align: "right" },
        { text: roe > 0 ? money(totalSar * roe) : "Pending", secondary: superseded ? "SUPERSEDED | REFERENCE ONLY" : undefined, align: "right", bold: true },
      ] } as TableRow;
    });
    hotelRows.push(...insertAdjustmentsAfterFirst(rows, booking.statementDisplayAdjustments, (adjustment, latest) => hotelAdjustmentRow(booking, adjustment, data, latest)));
  });
  y = renderSection(doc, data, "HOTEL BOOKINGS", hotelColumns, hotelRows, { label: "HOTEL STATEMENT SUBTOTAL", sar: effectiveSarTotal(data.sections.hotelBookings), pkr: statementSectionPeriodTotal(data.sections.hotelBookings), pendingSar: effectivePendingSar(data.sections.hotelBookings) }, BOOKING_THEME, y);
  ...
  data.sections.visaBookings.forEach((booking) => {
    const fallback = booking.lines.map((line) => {
      ...
      return { passengerType: line.passenger_type, passengerName: line.passenger_name, visaType: line.visa_type, visaRateSar: line.visa_rate_sar, qty: line.pax_count, roe: line.roe, transportPkrPerPax } as Obj;
    });
    const originals = hasOriginalSnapshot(booking) ? originalUniversalRows(booking, fallback) : fallback;
    const superseded = booking.statementDisplayAdjustments.length > 0;
    const rows = originals.map((line) => {
      visaSr += 1;
      const pax = Math.max(1, Number(line.qty || 1)), rateSar = Number(line.visaRateSar || 0), roe = Number(line.roe || 0), transportPkr = Number(line.transportPkrPerPax || 0) * pax;
      ...
      return { kind: superseded ? "reference" as const : "normal" as const, strike: superseded, cells: [
        ...
        { text: `Visa ${sar(rateSar)} / Pax`, secondary: transportPkr > 0 ? `Transport PKR ${money(transportPkr)}` : "Transport -" },
        { text: roe > 0 ? number(roe) : "Pending", align: "right" },
        { text: sar(totalSar), align: "right" },
        { text: totalPkr > 0 ? money(totalPkr) : "Pending", secondary: superseded ? "SUPERSEDED | REFERENCE ONLY" : undefined, align: "right", bold: true },
      ] } as TableRow;
    });
    visaRows.push(...insertAdjustmentsAfterFirst(rows, booking.statementDisplayAdjustments, (adjustment, latest) => visaAdjustmentRow(booking, adjustment, data, latest)));
  });
  y = renderSection(doc, data, "VISA BOOKINGS", visaColumns, visaRows, { label: "VISA STATEMENT SUBTOTAL", sar: effectiveSarTotal(data.sections.visaBookings), pkr: statementSectionPeriodTotal(data.sections.visaBookings), pendingSar: effectivePendingSar(data.sections.visaBookings) }, BOOKING_THEME, y);
  ...
  data.sections.transportBookings.forEach((booking) => {
    const fallback = booking.lines.map((line) => ({ transportDate: line.transport_date, transportType: line.transport_type, fromLocation: line.from_location, toLocation: line.to_location, vehicleType: line.vehicle_type, customVehicleName: line.custom_vehicle_name, vehicleCount: line.vehicle_count, rateSar: line.rate_sar, paxCount: line.pax_count, roe: line.roe } as Obj));
    const originals = hasOriginalSnapshot(booking) ? originalUniversalRows(booking, fallback) : fallback;
    const superseded = booking.statementDisplayAdjustments.length > 0;
    const rows = originals.map((line) => {
      transportSr += 1;
      const sharing = String(line.transportType) === "SHARING_BUS";
      const rateSar = Number(line.rateSar || 0), roe = Number(line.roe || 0), vehicleCount = Number(line.vehicleCount || 0), pax = Number(line.paxCount || 0);
      const totalSar = sharing ? rateSar * pax : rateSar * vehicleCount;
      const vehicle = sharing ? "Sharing Bus" : safeText(line.customVehicleName || titleCase(String(line.vehicleType || "")));
      return { kind: superseded ? "reference" as const : "normal" as const, strike: superseded, cells: [
        { text: String(transportSr), align: "center" },
        { text: shortDate(booking.transaction_date), secondary: displayUb(booking.ub_number), align: "center", bold: true },
        { text: `${safeText(line.fromLocation)} -> ${safeText(line.toLocation)}`, secondary: longDate(String(line.transportDate || booking.transaction_date)), bold: true },
        { text: sharing ? "Sharing Bus" : "Private Vehicle", secondary: vehicle },
        { text: sharing ? countLabel(pax, "Pax", "Pax") : countLabel(vehicleCount, "Vehicle"), secondary: sharing ? undefined : countLabel(pax, "Pax", "Pax") },
        { text: `${sar(rateSar)} / ${sharing ? "Pax" : "Vehicle"}`, secondary: roe > 0 ? `ROE ${number(roe)}` : "ROE Pending" },
        { text: sar(totalSar), align: "right" },
        { text: roe > 0 ? money(totalSar * roe) : "Pending", secondary: superseded ? "SUPERSEDED | REFERENCE ONLY" : undefined, align: "right", bold: true },
      ] } as TableRow;
    });
    transportRows.push(...insertAdjustmentsAfterFirst(rows, booking.statementDisplayAdjustments, (adjustment, latest) => transportAdjustmentRow(booking, adjustment, data, latest)));
  });
  y = renderSection(doc, data, "TRANSPORT BOOKINGS", transportColumns, transportRows, { label: "TRANSPORT STATEMENT SUBTOTAL", sar: effectiveSarTotal(data.sections.transportBookings), pkr: statementSectionPeriodTotal(data.sections.transportBookings), pendingSar: effectivePendingSar(data.sections.transportBookings) }, BOOKING_THEME, y);

  const miscColumns: Column[] = [
    { width: 7, header: "SR", align: "center" }, { width: 22, header: "DATE / UB", align: "center" },
    { width: 34, header: "SERVICE" }, { width: 36, header: "FAMILY HEAD" }, { width: 10, header: "PAX", align: "center" },
    { width: 39, header: "RATE / ROE" }, { width: 22, header: "ACCOUNT IMPACT", align: "right" },
    { width: 30, header: "TOTAL PKR", align: "right" },
  ];
  const miscRows: TableRow[] = [];
  let miscSr = 0;
  data.sections.miscBookings.forEach((booking) => {
    const fallback = booking.lines.map((line) => ({ serviceName: line.service_name, qty: line.pax_count, rate: line.rate_per_person, roe: line.currency_mode === "SAR" ? line.roe : 0 } as Obj));
    const originals = hasOriginalSnapshot(booking) ? originalUniversalRows(booking, fallback) : fallback;
    const superseded = booking.statementDisplayAdjustments.length > 0;
    const rows = originals.map((line, index) => {
      miscSr += 1;
      const qty = Math.max(1, Number(line.qty || 1)), rate = Number(line.rate || 0), roe = Number(line.roe || 0), base = rate * qty, total = roe > 0 ? base * roe : base;
      ...
      return { kind: superseded ? "reference" as const : "normal" as const, strike: superseded, cells: [
        ...
        { text: String(qty), align: "center" },
        { text: roe > 0 ? `SAR ${number(rate)} / Person` : `${money(rate)} / Person`, secondary: roe > 0 ? `ROE ${number(roe)}` : "PKR direct" },
        { text: "-", align: "right" },
        { text: money(total), secondary: superseded ? "SUPERSEDED | REFERENCE ONLY" : undefined, align: "right", bold: true },
      ] } as TableRow;
    });
    miscRows.push(...insertAdjustmentsAfterFirst(rows, booking.statementDisplayAdjustments, (adjustment, latest) => miscAdjustmentRow(booking, adjustment, data, latest)));
  });
  y = renderSection(doc, data, "MISC BOOKINGS", miscColumns, miscRows, { label: "MISC STATEMENT SUBTOTAL", sar: effectiveSarTotal(data.sections.miscBookings), pkr: statementSectionPeriodTotal(data.sections.miscBookings), pendingSar: effectivePendingSar(data.sections.miscBookings) }, BOOKING_THEME, y);
  ...
  const paymentRows: TableRow[] = data.payments.map((entry, index) => ({ cells: [
