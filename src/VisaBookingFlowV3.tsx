...
import { num, whole, visaVehicleCapacity, visaNeedsPrivate, visaNeedsBus, calculateVisaSummary } from "./pricingEngines";
...
export default function VisaBookingFlowV3({ companyId, parties, transactionType, userId = "", canCreate = true, canEdit = true, canVoid = true, onBack, onChanged }: Props) {
  ...
  const { mode, setMode, tx, setTx, counterpartyId, setCounterpartyId, bookingDate, setBookingDate, ubDigits, setUbDigits, ubNumber, setUbNumber, assigned, setAssigned, saved, setSaved, detailsOpen, setDetailsOpen, editingId, setEditingId, busy, setBusy, error, setError, message, setMessage, assignUb: assign, resetState } = useBookingFlowState(companyId, transactionType, entries, "Visa");
  ...
  function reset() {
    resetState();
    setRows([newVisaRow()]); setFleet([]); setFleetTouched(false); setBusRate(""); setExpectedEntryDate(""); setPassengers([]); setNotes("");
    setLegacyPassports([]); setLegacyExpectedEntry(""); setLegacyNotes("");
  }
