...
export default function PackageBookingFlowV2({ companyId, parties, transactionType, userId = "", canCreate = true, canEdit = true, canVoid = true, onBack, onChanged }: Props) {
  ...
  const saleTotal = accountingEntries.filter((entry) => entry.transaction_type === "SALE").reduce((sum, entry) => sum + Number(entry.total_pkr || 0), 0);
  const purchaseTotal = accountingEntries.filter((entry) => entry.transaction_type === "PURCHASE").reduce((sum, entry) => sum + Number(entry.total_pkr || 0), 0);
  const registerPax = accountingEntries.reduce((sum, entry) => sum + entry.lines.reduce((lineSum, line) => lineSum + Number(line.person_count || 0), 0), 0);
  ...
  async function saveCommercial() {
    ...
    if (editingId) return setError("Saved Package commercial values are changed through Booking Adjustment so every correction/amendment keeps history.");
    ...
    try {
      ...
      setMessage(`Package booking ${ubNumber} saved successfully. Commercial values are now locked; use Booking Adjustment for future corrections, amendments or cancellations.`);
  ...
  async function saveQuickAccount() {
    ...
    try {
      const input: PartyInput = {
        name: quickAccount.name.trim(), phone: quickAccount.phone.trim(), whatsapp: quickAccount.whatsapp.trim(),
        address: quickAccount.address.trim(), notes: quickAccount.notes.trim(), status: "ACTIVE",
  ...
  function openEntry(entry: PackageBooking) {
    ...
    setRows(entry.lines.length ? entry.lines.map((line) => ({
      rowId: crypto.randomUUID(), passengerType: line.passenger_type, passengerName: line.passenger_name || "",
      packageType: line.package_type, rate: String(line.rate_per_person), count: String(Math.max(1, Number(line.person_count || 1))),
    })) : []);
    setEditingId(entry.id);
    setMode("FORM");
    setError("");
    setMessage(`Opened ${entry.ub_number}. Commercial rows are read-only here. Use Booking Adjustment from the Register for Correction, Amendment or Cancellation.`);
    window.scrollTo({ top: 0, behavior: "smooth" });
  }

  async function voidEntry(entry: PackageBooking) {
    if (!canVoid || entry.status !== "ACTIVE" || busy) return;
    if (!window.confirm(`Void Package booking ${entry.ub_number}? Use Void only when this booking should never have existed. The audit record will remain.`)) return;
    setBusy(true);
    setError("");
    try {
  ...
  function renderForm() {
    ...
    return <section className="booking-entry-screen package14-page">
      ...
      <div className="package14-title"><div><span className="eyebrow blue">PACKAGE BOOKING</span><h2>{commercialSaved ? `Package Booking — ${ubNumber}` : "New Package Booking"}</h2><p>{editingId ? "Review the current effective Package booking. Commercial changes are protected by Booking Adjustment history." : "Create the UB first, save the accounting Package values second, then complete optional passenger/travel operations when required."}</p></div></div>
      {message && <div className="alert success">{message}</div>}{error && <div className="alert error">{error}</div>}

      {!ubAssigned ? <section className="package14-identity">
      ...
      </section> : <section className={`package14-identity-complete ${commercialSaved ? "saved" : "ready"}`}>
        <span className="package14-check">✓</span><div className="package14-identity-main"><small>{commercialSaved ? "PACKAGE BOOKING" : "BOOKING UB READY"}</small><b>{ubNumber}</b><span>{selectedAccount?.name || accountNoun}</span></div><div><small>BOOKING DATE</small><b>{bookingDate}</b></div><div><small>TRANSACTION</small><b>{activeTransactionType}</b></div><div><small>LIFECYCLE</small><b>{commercialSaved ? `${openedLifecycle} · REV ${openedRevision}` : "READY"}</b></div>{!commercialSaved ? <button type="button" className="secondary" onClick={() => { setUbAssigned(false); setMessage(""); }}>Edit Booking Header</button> : <span className="package14-lock">UB/account identity locked</span>}
      </section>}

      {ubAssigned && <section className="package14-commercial-card">
        <div className="package14-section-heading"><span>02</span><div><b>PACKAGE DETAILS & RATES</b><small>{editingId ? "Current effective commercial rows — use Booking Adjustment to change them." : `Commercial / accounting data saved under ${ubNumber}.`}</small></div></div>
        ...
        <div className="package14-summary"><div className="package14-summary-title"><span>PACKAGE BOOKING SUMMARY</span><small>{editingId ? "Current row base. Register total may also include amendment/cancellation charges or credits." : "Part of Section 02 — these totals form the Package accounting value."}</small></div><div className="package14-summary-body"><div className="package14-breakdown"><div><span>Adults</span><b>{totals.qty.ADULT}</b><strong>{money(totals.amount.ADULT)}</strong></div><div><span>Children</span><b>{totals.qty.CHILD}</b><strong>{money(totals.amount.CHILD)}</strong></div><div><span>Infants</span><b>{totals.qty.INFANT}</b><strong>{money(totals.amount.INFANT)}</strong></div></div><div className="package14-grand"><div><small>TOTAL ACTIVE PAX</small><b>{totals.totalPax}</b></div><div><small>ACTIVE ROW BASE</small><strong>{money(totals.grandTotal)}</strong></div></div></div></div>
        <div className="package14-commercial-actions">{commercialSaved && <button type="button" className="secondary" onClick={resetForm}>+ New Package Booking</button>}{!editingId && canCreate && <button type="button" className="primary package14-save" disabled={busy} onClick={() => void saveCommercial()}>{busy ? "Saving..." : `Save Package Booking — ${ubNumber}`}</button>}{editingId && canEdit && openedLifecycle !== "CANCELLED" && <button type="button" className="primary package14-save" onClick={() => { const entry = entries.find((item) => item.id === editingId); if (entry) { setAdjustmentBooking(entry); setMode("REGISTER"); } }}>Open Booking Adjustment</button>}</div>
      </section>}

      {commercialSaved && editingId && <section className={`package14-additional package15-shell ${detailsOpen ? "open" : "closed"}`}><button type="button" className="package14-additional-toggle" onClick={() => setDetailsOpen((value) => !value)}><span className="package14-step-purple">03</span><div><b>PACKAGE TRAVEL & PASSENGER DETAILS — {ubNumber}</b><small>Operational details remain separate from accounting values.</small></div><span className="package14-optional">OPTIONAL</span><strong>{detailsOpen ? "Close Details ▲" : "+ Open Details ▼"}</strong></button>{detailsOpen && <PackageOperationalDetails companyId={companyId} bookingId={editingId} ubNumber={ubNumber} adultPax={totals.qty.ADULT} childPax={totals.qty.CHILD} infantPax={totals.qty.INFANT} canEdit={canEdit && openedLifecycle !== "CANCELLED"} onSaved={async () => { await loadEntries(search); }} />}</section>}

      {!ubAssigned && <div className="package14-next-step">Create / Assign a Booking UB to unlock Package Details & Rates.</div>}
      {ubAssigned && !commercialSaved && <div className="package14-next-step">Save Section 02 to activate the Package booking.</div>}
  ...
  function renderRegister() {
    return <section className="booking-entry-screen package14-page package14-register-page">
      ...
      <div className="package14-register-title"><div><span className="eyebrow blue">PACKAGE BOOKING REGISTER</span><h2>Package Booking Register</h2><p>Original bookings stay tied to their genuine UB. Correction, Amendment and Cancellation are stored as revision history.</p></div><div className="package14-register-stats"><div><small>LIVE BOOKINGS</small><b>{liveBookings.length}</b></div><div><small>SALES</small><b>{money(saleTotal)}</b></div><div><small>PURCHASES</small><b>{money(purchaseTotal)}</b></div><div><small>ACTIVE PAX</small><b>{registerPax}</b></div></div></div>
      {message && <div className="alert success">{message}</div>}{error && <div className="alert error">{error}</div>}
      ...
      {visibleEntries.length === 0 ? <div className="empty-state compact-empty"><div className="empty-icon">PKG</div><h3>No Package bookings found</h3><p>Create a Package booking or change the filter/search.</p></div> : <div className="party-table-wrap package14-register-wrap"><table className="party-table package14-register-table"><thead><tr><th>DATE</th><th>UB #</th><th>TYPE</th><th>PARTY / VENDOR</th><th>CURRENT PACKAGE ROWS</th><th>PAX</th><th>EFFECTIVE TOTAL PKR</th><th>SECTION 03</th><th>LIFECYCLE</th><th>ACTIONS</th></tr></thead><tbody>{visibleEntries.map((entry) => {
        ...
        return <tr key={entry.id} className={entry.status === "VOID" ? "void-row" : ""}><td>{entry.transaction_date}</td><td><b>{entry.ub_number}</b></td><td><span className={`direction-badge ${entry.transaction_type === "SALE" ? "sale" : "purchase"}`}>{entry.transaction_type}</span></td><td><b>{entry.counterparty_name || "—"}</b></td><td><div className="package14-register-lines">{entry.lines.length ? entry.lines.map((line) => <div key={line.id}><span className={`passenger-chip ${line.passenger_type.toLowerCase()}`}>{line.passenger_type}</span><b>{line.passenger_name}</b><span>{line.package_type}</span><small>{line.person_count} × {money(line.rate_per_person)} = {money(line.line_total_pkr)}</small></div>) : <span>All commercial rows cancelled</span>}</div></td><td><b>{pax}</b></td><td className="amount"><b>{money(entry.total_pkr)}</b></td><td><span className="package14-detail-status optional">Optional</span></td><td><span className={`status package14-lifecycle ${lifecycleClass}`}>{lifecycle} · REV {revision}</span></td><td><div className="row-actions"><button type="button" disabled={entry.status !== "ACTIVE" || busy} onClick={() => openEntry(entry)}>Open Booking</button><button type="button" className="package14-adjustment-action" disabled={!canEdit || entry.status !== "ACTIVE" || fullyCancelled || busy} onClick={() => setAdjustmentBooking(entry)}>Booking Adjustment</button><button type="button" className="package14-history-action" disabled={entry.status === "VOID" && !summary} onClick={() => setHistoryBooking(entry)}>History</button><button type="button" disabled={!canVoid || entry.status !== "ACTIVE" || busy} onClick={() => void voidEntry(entry)}>Void</button></div></td></tr>;
      })}</tbody></table></div>}
    </section>;
  }
  ...
  return <>
    ...
    {adjustmentBooking && <PackageBookingAdjustment companyId={companyId} booking={adjustmentBooking} userId={userId} canEdit={canEdit} initialView="ADJUSTMENT" onClose={() => setAdjustmentBooking(null)} onSaved={adjustmentSaved} />}
    {historyBooking && <PackageBookingAdjustment companyId={companyId} booking={historyBooking} userId={userId} canEdit={canEdit} initialView="HISTORY" onClose={() => setHistoryBooking(null)} onSaved={adjustmentSaved} />}
  </>;
}
