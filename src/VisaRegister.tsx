...
const config = bookingLifecycleConfigs.VISA;

const visaLabels: Record<VisaType, string> = {
  ONLY_UMRAH_VISA: "Only Umrah Visa",
  UMRAH_VISA_TRANSPORT: "Umrah Visa + Transport",
  UMRAH_VISA_ONE_WAY_TRANSPORT: "Umrah Visa + One-Way Transport",
  UMRAH_VISA_FULL_TRANSPORT: "Umrah Visa + Full Transport",
};
...
function visaLabel(type: VisaType) {
  return visaLabels[type] || type.replace(/_/g, " ");
}

function bookingSummary(booking: VisaBooking) {
  return (
    booking.lines
      .map((line) => `${line.passenger_name} · ${visaLabel(line.visa_type)} · ${line.pax_count} pax`)
      .join(" | ") || "Visa booking"
  );
}
...
export default function VisaRegister({
  ...
  const [summaries, setSummaries] = useState<Record<string, VisaAdjustmentSummary>>({});
  ...
  async function load(nextSearch = search) {
    try {
      const [raw, nextSummaries] = await Promise.all([
        getVisaBookings(companyId, nextSearch),
  ...
  async function voidBooking(booking: VisaBooking) {
    ...
    setError("");
    try {
      await voidVisaBooking(companyId, booking.id, userId);
      await load();
      await onChanged?.();
  ...
  async function doDelete(booking: VisaBooking) {
    if (
      !window.confirm(
  ...
  async function openBooking(booking: VisaBooking, lifecycle: BookingLifecycleStatus) {
    ...
    <>
      <section className="booking-entry-screen package14-page package14-register-page lifecycle-register-page">
        ...
        <div className="package14-register-title">
          <div>
            <span className="eyebrow blue">VISA BOOKING REGISTER</span>
            <h2>{config.label} Booking Register</h2>
            <p>
              Open Booking, Booking Adjustment, History and Void Booking stay in the same Actions column. Visa
              adjustments are stored in dedicated Visa revision history.
            </p>
          </div>
        ...
        {!visible.length ? (
          <div className="empty-state compact-empty">
            <div className="empty-icon">VSA</div>
                          ...
                          {booking.lines.length ? (
                            booking.lines.map((line) => (
                              <div key={line.id}>
                                ...
                                <span>
                                  {line.passenger_type} · {visaLabel(line.visa_type)}
                                </span>
                                <small>
                                  {line.pax_count} × {sar(line.visa_rate_sar)} = {money(line.line_total_pkr)}
                                </small>
                              </div>
                            ))
                          ) : (
        ...
        >
          <section className="adj-shell lifecycle-preview" onMouseDown={(e) => e.stopPropagation()}>
            <div className="adj-toolbar">
              <div>
                <span className="eyebrow blue">OPEN VISA BOOKING</span>
            ...
            <section className="adj-section">
              <div className="adj-section-title">
                <span>02</span>
                <div>
                  <b>CURRENT VISA COMMERCIAL ROWS</b>
                  ...
                  <tbody>
                    {previewBooking.lines.map((line) => (
                      <tr key={line.id}>
                        ...
                        <td>{sar(line.visa_rate_sar)}</td>
                        <td>{line.pax_count}</td>
