...
function ServiceIcon({ service }: { service: BookingService }) {
  ...
  if (service === "PACKAGE") return <svg {...common}><rect x="5" y="7" width="14" height="12" rx="2" /><path d="M9 7V5.8A1.8 1.8 0 0 1 10.8 4h2.4A1.8 1.8 0 0 1 15 5.8V7" /><path d="M5 11h14M9 11v2M15 11v2" /></svg>;
  if (service === "TICKET") return <svg {...common}><path d="M4 8.5A2.5 2.5 0 0 0 6.5 6h11.2A1.3 1.3 0 0 1 19 7.3V10a2 2 0 0 0 0 4v2.7a1.3 1.3 0 0 1-1.3 1.3H6.5A2.5 2.5 0 0 0 4 15.5z" /><path d="M13 8.5v1M13 12v1M13 15.5v1" /></svg>;
  if (service === "HOTEL") return <svg {...common}><path d="M5 20V5.5A1.5 1.5 0 0 1 6.5 4h8A1.5 1.5 0 0 1 16 5.5V20" /><path d="M16 9h2.5A1.5 1.5 0 0 1 20 10.5V20M3 20h18" /><path d="M8 8h2M8 11h2M8 14h2M13 8h1M13 11h1M13 14h1" /></svg>;
  if (service === "VISA") return <svg {...common}><rect x="5" y="3" width="14" height="18" rx="2" /><circle cx="12" cy="10" r="3" /><path d="M9 17h6M8 6h8" /></svg>;
  if (service === "TRANSPORT") return <svg {...common}><rect x="4" y="5" width="16" height="13" rx="3" /><path d="M7 18v2M17 18v2M4 13h16M7 8h4M14 8h3" /><circle cx="8" cy="16" r="1" /><circle cx="16" cy="16" r="1" /></svg>;
...
export default function BookingsModule({ companyId, parties, userId = "", canCreate = true, canEdit = true, canVoid = true, onChanged }: Props) {
  ...
  function chooseDirection(next: BookingTransactionType) { setTransactionType(next); setService(null); setScreen("SERVICES"); }
  ...
  function renderDirectionScreen() {
    return <section className="booking-entry-screen booking-direction-screen">
      ...
      <div className="booking-direction-grid">
        ...
        <button type="button" className="booking-direction-card purchase" onClick={() => chooseDirection("PURCHASE")}><span className="direction-card-icon" aria-hidden="true">↙</span><div><small>PURCHASE</small><b>Purchase from Vendor / Supplier</b><p>Record a booking purchased from a Vendor / supplier account.</p></div><span className="direction-arrow">→</span></button>
      </div>
    </section>;
  }
