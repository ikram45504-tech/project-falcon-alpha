      ...
      for (const entry of allRows) {
        if (entry.row.kind === "SALE_BOOKING") {
          ...
          if (existing && existing.partyId !== entry.partyId) {
            setErrors(prev => [...prev, `[VIOLATION] ${entry.row.ref_no} is assigned as a SALE to both "${existing.partyName}" and "${entry.partyName}".`]);
            issues++;
          } else {
    ...
    <div className="security-form" style={{ maxWidth: 800 }}>
      <div className="form-head">
        <div>
          ...
          <p>Scan the database for logical errors, missing links, and UB ownership violations across all booking modules.</p>
        </div>
      </div>
      ...
      {results.length > 0 && (
        <div style={{ marginTop: 24, padding: 16, backgroundColor: "var(--bg)", borderRadius: 8, border: "1px solid var(--border)" }}>
          ...
          <ul style={{ margin: 0, paddingLeft: 20, fontSize: 14, color: "var(--text)" }}>
            {results.map((res, i) => <li key={i} style={{ marginBottom: 4 }}>{res}</li>)}
          </ul>
        </div>
      )}

      {errors.length > 0 && (
        <div style={{ marginTop: 24, padding: 16, backgroundColor: "var(--bg)", borderRadius: 8, border: "1px solid var(--red)", color: "var(--red)" }}>
      ...
      <div style={{ marginTop: 40, paddingTop: 24, borderTop: "1px solid var(--border)" }}>
        ...
        <p style={{ marginBottom: 16, fontSize: 14 }}>Wipe all booking, payment, and party data from this company to start fresh. This action cannot be undone.</p>
        <button 
          type="button" 
          className="primary" 
          style={{ backgroundColor: "var(--red)" }}
          onClick={async () => {
            if (window.confirm("Are you ABSOLUTELY sure you want to erase all data? This will wipe all parties, bookings, and payments.")) {
