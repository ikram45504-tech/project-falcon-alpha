...
import type { TransportBooking, TransportType, TransportVehicleType } from "./db";
import {
  getTransportAdjustmentHistory,
  saveTransportCancellation,
  saveTransportCorrectionOrAmendment,
  type TransportAdjustmentRecord,
  type TransportAdjustmentType,
} from "./TransportAdjustmentDb";
import { bookingAccountTerms, bookingLifecycleConfigs } from "./BookingLifecycle";
import { transportRowCalc } from "./pricingEngines";
...
const amendmentCategories = bookingLifecycleConfigs.TRANSPORT.amendmentTypes;
...
function lineTotalPkr(row: RowState) {
  return transportRowCalc(row).totalPkr;
}

function lineCancelQty(line: TransportBooking["lines"][number]) {
...
function rowFromBooking(line: TransportBooking["lines"][number]): RowState {
  return {
    ...
    vehicleCount: String(Math.max(1, Number(line.vehicle_count || 1))),
    rateSar: String(Number(line.rate_sar || 0)),
...
function adjustmentLabel(type: TransportAdjustmentType) {
...
export default function TransportBookingAdjustment({
  companyId,
  booking,
  userId = "",
  canEdit = true,
  ...
  const [adjustmentType, setAdjustmentType] = useState<TransportAdjustmentType | "">("");
  ...
  const [history, setHistory] = useState<TransportAdjustmentRecord[]>([]);
  ...
  async function loadHistory() {
    try {
      setHistory(await getTransportAdjustmentHistory(companyId, booking.id));
      setError("");
    } catch (e) {
  ...
  const cancelledValue = useMemo(() => {
    ...
    return booking.lines.reduce((sum, line) => {
      const available = lineCancelQty(line);
  ...
  function chooseType(type: TransportAdjustmentType) {
  ...
  async function save() {
    ...
    try {
      if (adjustmentType === "CORRECTION" || adjustmentType === "AMENDMENT") {
        const result = await saveTransportCorrectionOrAmendment(
          companyId,
          booking.id,
          {
            adjustmentType,
            ...
            lines: rows.map((row) => ({
              ...
              customVehicleName: row.customVehicleName.trim(),
              vehicleCount: Math.max(1, Math.trunc(numberValue(row.vehicleCount))),
              rateSar: numberValue(row.rateSar),
              paxCount: Math.max(1, Math.trunc(numberValue(row.paxCount))),
              roe: numberValue(row.roe) > 0 ? numberValue(row.roe) : null,
            })),
          },
          userId,
        );
        await onSaved?.(
          `${adjustmentLabel(adjustmentType)} saved for ${booking.ub_number}. ${accountNoun} adjustment: ${signedMoney(result.delta)}. Current Transport value: ${money(result.effectiveTotal)}.`,
        );
      } else {
        const result = await saveTransportCancellation(
          companyId,
          booking.id,
          {
            adjustmentType,
        ...
        await onSaved?.(
          `${adjustmentLabel(adjustmentType)} saved for ${booking.ub_number}. Account credit: ${money(result.accountCredit)}. Current Transport value: ${money(result.effectiveTotal)}.`,
        );
      }
      onClose();
    } catch (e) {
  ...
  function renderSelection() {
    const choices: Array<{ type: TransportAdjustmentType; title: string; text: string; badge: string }> = [
      {
        type: "CORRECTION",
        title: "Correction",
      ...
      },
      {
        ...
        text: "Cancel selected Transport sectors / quantities and calculate cancellation charges and account credit.",
        badge: "SELECT ITEMS",
      },
      {
        type: "FULL_CANCELLATION",
        title: "Full Cancellation",
        text: "Cancel the complete Transport booking while retaining any applicable cancellation charge.",
        badge: "FULL BOOKING",
      },
    ];
    return (
    ...
    return (
      <>
        <div className="adj-add-row-actions">
          <span>Revised Transport Rows</span>
          <div>
                ...
                <tr key={row.rowId}>
                  <td>
                    <input
                      ...
                      onChange={(e) => updateRow(row.rowId, { transportDate: e.target.value })}
                    />
                  </td>
                  <td>
                    <select
                      value={row.transportType}
                      onChange={(e) => updateRow(row.rowId, { transportType: e.target.value as TransportType })}
                    >
                  ...
                  <td>
                    <input
                      ...
                      onChange={(e) => updateRow(row.rowId, { fromLocation: e.target.value })}
                    />
                  </td>
                  <td>
                    <input
                      value={row.toLocation}
                      onChange={(e) => updateRow(row.rowId, { toLocation: e.target.value })}
                    />
                  </td>
                  <td>
                    {row.transportType === "SHARING_BUS" ? (
                      <span>Sharing Bus</span>
                    ) : (
                      <div className="adj-inline-stack">
                        <select
                          value={row.vehicleType}
                          onChange={(e) =>
                            updateRow(row.rowId, { vehicleType: e.target.value as TransportVehicleType })
                          }
                        >
                          {vehicles.map((option) => (
                            <option value={option.value} key={option.value}>
                        ...
                        {row.vehicleType === "OTHER" && (
                          <input
                            ...
                            onChange={(e) => updateRow(row.rowId, { customVehicleName: e.target.value })}
                  ...
                  <td>
                    <input
                      ...
                      step="0.01"
                      value={row.rateSar}
                      onChange={(e) => updateRow(row.rowId, { rateSar: e.target.value })}
                    />
                  </td>
                  <td>
                    {row.transportType === "SHARING_BUS" ? (
                      ...
                      <input
                        ...
                        onChange={(e) => updateRow(row.rowId, { vehicleCount: e.target.value })}
                      />
                    )}
                  </td>
                  <td>
            ...
            {booking.lines.map((line) => {
              ...
              return (
                <tr key={line.id}>
                  ...
                  <td>
                    <b>{line.to_location}</b>
                  </td>
                  <td>
                    {line.transport_type === "SHARING_BUS"
                      ? "Sharing Bus"
                      : line.custom_vehicle_name || line.vehicle_type.replace(/_/g, " ")}
                  </td>
                  <td>{available}</td>
                  <td>
                    {adjustmentType === "FULL_CANCELLATION" ? (
      ...
      <div className="adj-history-view">
        ...
        <div className="adj-timeline">
          <article className="adj-history-item original">
            ...
            <div>
              ...
              <h4>Original Transport Booking</h4>
              <p>
        ...
        {!history.length && (
          <div className="adj-empty-history">
            No booking adjustments yet. This is still the original Transport booking.
          </div>
        )}
      </div>
    );
                    ...
                    <div className="adj-form-grid">
                      <label>
                        ...
                        <input
                          ...
                          placeholder="Driver / supplier / internal reference"
                        />
                      </label>
                      <label className="wide">
