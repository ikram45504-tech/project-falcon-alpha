...
export default function TransportBookingAdjustment({
  ...
  const availableChargeQty = useMemo(
    () => booking.lines.reduce((sum, line) => sum + lineCancelQty(line), 0),
    [booking.lines],
  );
  ...
  const chargeUnitLabel = adjustmentChargeUnitLabel("TRANSPORT");
  ...
  async function save() {
    ...
    try {
      if (adjustmentType === "CORRECTION" || adjustmentType === "AMENDMENT") {
        ...
        await onSaved?.(
          `${adjustmentTypeLabel(adjustmentType)} saved for ${booking.ub_number}. ${accountNoun} adjustment: ${signedMoney(result.delta)}. Current Transport value: ${money(result.effectiveTotal)}.`,
        );
      } else {
        ...
        await onSaved?.(
          `${adjustmentTypeLabel(adjustmentType)} saved for ${booking.ub_number}. Account credit: ${money(result.accountCredit)}. Current Transport value: ${money(result.effectiveTotal)}.`,
        );
      }
      onClose();
    } catch (e) {
  ...
  function renderSelection() {
    const choices = buildAdjustmentChoices(
      bookingLifecycleConfigs.TRANSPORT.label,
      bookingLifecycleConfigs.TRANSPORT.partialCancellationLabel,
    );
    return (
