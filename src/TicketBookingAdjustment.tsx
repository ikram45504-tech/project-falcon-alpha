...
export default function TicketBookingAdjustment({
  ...
  const availableChargeQty = useMemo(
    () => booking.lines.reduce((sum, line) => sum + Math.max(1, Math.trunc(Number(line.ticket_count || 1))), 0),
    [booking.lines],
  );
  ...
  const chargeUnitLabel = adjustmentChargeUnitLabel("TICKET");
  ...
  async function save() {
    ...
    try {
      if (adjustmentType === "CORRECTION" || adjustmentType === "AMENDMENT") {
        ...
        await onSaved?.(
          `${adjustmentTypeLabel(adjustmentType)} saved for ${booking.ub_number}. ${accountNoun} adjustment: ${signedMoney(result.delta)}. Current Ticket value: ${money(result.effectiveTotal)}.`,
        );
      } else {
        ...
        await onSaved?.(
          `${adjustmentTypeLabel(adjustmentType)} saved for ${booking.ub_number}. Account credit: ${money(result.accountCredit)}. Current Ticket value: ${money(result.effectiveTotal)}.`,
        );
      }
      onClose();
    } catch (e) {
  ...
  function renderSelection() {
    const choices = buildAdjustmentChoices(
      bookingLifecycleConfigs.TICKET.label,
      bookingLifecycleConfigs.TICKET.partialCancellationLabel,
    );
    return (
