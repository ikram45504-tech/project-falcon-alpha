...
export default function TicketBookingFlowV2({ companyId, parties, transactionType, userId = "", canCreate = true, canEdit = true, canVoid = true, onBack, onChanged }: Props) {
  ...
  async function loadEntries() {
    try { setEntries(await getTicketCommercialBookings(companyId)); }
  ...
  function removeRow(id: string) {
    if (commercialLocked) return;
    setRows((current) => { const next = current.filter((row) => row.rowId !== id); return next.length ? next : [newRow()]; });
  }
  ...
  async function saveCommercial() {
    if (editingId) return setError("Commercial Ticket values are locked after saving. Use Ticket Booking Register → Booking Adjustment for Correction, Amendment or Cancellation.");
    ...
    try {
      ...
      await loadEntries(); await onChanged?.();
    } catch (e) { setError(e instanceof Error ? e.message : String(e)); }
  ...
  function openEntry(entry: TicketCommercialBooking) {
    ...
    setEditingId(entry.id); setMode("FORM"); setError(""); setMessage(`Opened Ticket booking ${entry.ub_number}. Commercial values are read-only here; use Booking Adjustment from the Ticket Booking Register. Section 03 operational details remain available.`); window.scrollTo({ top: 0, behavior: "smooth" });
  }
  ...
  return <section className="booking-entry-screen ticket9-screen package14-page">
    <div className="booking-screen-toolbar ticket9-toolbar"><button className="booking-back-button" onClick={onBack}>← Back to Booking Services</button><div className="ticket9-toolbar-right"><span className={`direction-badge ${tx === "SALE" ? "sale" : "purchase"}`}>{tx === "SALE" ? "SALE TO PARTY" : "PURCHASE FROM VENDOR / SUPPLIER"}</span><button className="ticket9-register-button" onClick={() => setMode("REGISTER")}>Ticket Booking Register</button></div></div>
    <div className="ticket9-title-row"><div><span className="eyebrow blue">TICKET BOOKING</span><h2>{saved ? `Ticket Booking — ${ubNumber}` : "New Ticket Booking"}</h2><p>{editingId ? "Review the current effective Ticket booking. Commercial changes are protected by Booking Adjustment history." : "Create the UB first, save Ticket fares second, then add optional passenger and flight details."}</p></div></div>
    {message && <div className="alert success">{message}</div>}{error && <div className="alert error">{error}</div>}

    <ProgressiveBookingIdentity companyId={companyId} userId={userId} transactionType={tx} parties={parties} counterpartyId={counterpartyId} onCounterpartyChange={setCounterpartyId} bookingDate={bookingDate} onBookingDateChange={setBookingDate} ubDigits={ubDigits} onUbDigitsChange={setUbDigits} ubNumber={ubNumber} assigned={assigned} saved={saved} onAssign={assign} onEditHeader={() => { if (!editingId) { setAssigned(false); setMessage(""); } }} onAccountsChanged={onChanged} onError={setError} onMessage={setMessage} serviceLabel="Ticket" />

    {assigned && <section className="ticket9-card">
      <div className="ticket9-section-head"><span>2</span><b>TICKET DETAILS & FARES</b><small>{commercialLocked ? "Current effective commercial rows — use Booking Adjustment to change them." : `Commercial / accounting data under ${ubNumber}`}</small></div>
      <div className="ticket17-fare-head">{!commercialLocked && <button type="button" className="ticket17-add-row" disabled={!canCreate} onClick={addRow}>+ Ticket Row</button>}</div>
      <div className="ticket17-fare-wrap"><table className="ticket17-fare-table"><thead><tr><th>SR</th><th>PAX TYPE</th><th>PASSENGER / FAMILY HEAD</th><th>AIRLINE</th><th>PNR</th><th>FLIGHT TYPE</th><th>TICKET ROUTE</th><th>RATE / TICKET (PKR)</th><th>QTY</th><th>SUB TOTAL</th>{!commercialLocked && <th>ACTION</th>}</tr></thead><tbody>{rows.map((row, index) => <tr key={row.rowId}>
        <td className="ticket17-sr">{index + 1}</td>
        <td><select disabled={commercialLocked} value={row.passengerType} onChange={(e) => updateRow(row.rowId, { passengerType: e.target.value as TicketPassengerType })}><option value="ADULT">Adult</option><option value="CHILD">Child</option><option value="INFANT">Infant</option></select></td>
        <td><input disabled={commercialLocked} value={row.passengerName} onChange={(e) => updateRow(row.rowId, { passengerName: e.target.value })} placeholder="Passenger / Family Head" /></td>
        <td><input disabled={commercialLocked} value={row.airlineName} onChange={(e) => updateRow(row.rowId, { airlineName: e.target.value })} placeholder="Airline" /></td>
        <td><input disabled={commercialLocked} className="ticket17-pnr-input" value={row.pnr} onChange={(e) => updateRow(row.rowId, { pnr: e.target.value.toUpperCase() })} placeholder="PNR" /></td>
        <td><select disabled={commercialLocked} className="ticket17-flight-type-select" value={row.flightType} onChange={(e) => updateRow(row.rowId, { flightType: e.target.value as TicketFareFlightType })}><option value="ONE_WAY">One Way</option><option value="RETURN">Return</option><option value="MULTI_CITY">Multi-City</option></select></td>
      ...
      <div className="ticket17-commercial-actions">{!editingId && canCreate && <button type="button" className="primary" disabled={busy} onClick={() => void saveCommercial()}>{busy ? "Saving..." : `Save Ticket Booking — ${ubNumber}`}</button>}{editingId && <div className="adj-rule-note"><b>Commercial values locked:</b> use Ticket Booking Register → Booking Adjustment for Correction, Amendment, Partial Cancellation or Full Cancellation.</div>}</div>
    </section>}
