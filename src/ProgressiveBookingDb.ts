...
async function audit(companyId: string, userId: string, module: string, bookingId: string, details: string) {
  ...
  await database.execute(
    ...
    [crypto.randomUUID(), companyId, userId, users[0]?.full_name || "Unknown User", module, bookingId, details, new Date().toISOString()]
  );
}

export async function updateTicketAdditionalDetails(companyId: string, bookingId: string, input: {
  flightNo: string;
  ...
  await database.execute(
    ...
    [input.flightNo.trim(), input.departureTime, input.arrivalTime, input.baggage.trim(), input.ticketStatus, input.customerContact.trim(), input.notes.trim(), new Date().toISOString(), userId, bookingId, companyId]
  );
  await audit(companyId, userId, "TICKET", bookingId, "Ticket operational details updated without changing fare totals.");
}

export async function updateHotelAdditionalDetails(companyId: string, bookingId: string, input: {
  ...
  await database.execute(
    `UPDATE hotel_bookings SET confirmation_voucher=$1,meal_plan=$2,guest_family_name=$3,guest_count=$4,customer_contact=$5,special_requests=$6,notes=$7,updated_at=$8,updated_by_user_id=$9
     WHERE id=$10 AND company_id=$11 AND status='ACTIVE'`,
    [input.confirmationVoucher.trim(), input.mealPlan.trim(), input.guestFamilyName.trim(), Math.max(0, Math.trunc(input.guestCount || 0)), input.customerContact.trim(), input.specialRequests.trim(), input.notes.trim(), new Date().toISOString(), userId, bookingId, companyId]
  );
  await audit(companyId, userId, "HOTEL", bookingId, "Hotel reservation details updated without changing hotel totals.");
}

export async function updateTransportAdditionalDetails(companyId: string, bookingId: string, input: { paxSaudiNumber: string; notes: string }, userId = "") {
  await requireEdit(companyId, userId);
  const database = await db();
  await database.execute(
  ...
  await audit(companyId, userId, "TRANSPORT", bookingId, "Transport operational details updated without changing transport totals.");
}

export async function updateVisaAdditionalDetails(companyId: string, bookingId: string, input: {
  ...
  for (const [index, passenger] of input.passports.entries()) {
    ...
    await database.execute(
      ...
      [crypto.randomUUID(), bookingId, passenger.sourceFamilyName.trim(), passengerName, passenger.passengerType, passenger.visaType, passenger.surname.trim(), passenger.givenName.trim(), passenger.passportNumber.trim(), passenger.nationality.trim(), passenger.dateOfBirth, passenger.passportIssuance, passenger.passportExpiry, index]
    );
  }
...
export async function getMiscAdditionalDetails(companyId: string, bookingId: string) {
  ...
  const database = await db();
  const rows = await database.select<Array<{ notes: string }>>(`SELECT notes FROM misc_booking_details WHERE company_id=$1 AND booking_id=$2 LIMIT 1`, [companyId, bookingId]);
  return rows[0]?.notes || "";
}
