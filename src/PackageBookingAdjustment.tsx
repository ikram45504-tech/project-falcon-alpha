...
export default function PackageBookingAdjustment({
  ...
  const availableChargeQty = useMemo(
    () => booking.lines.reduce((sum, line) => sum + Math.max(1, Math.trunc(Number(line.person_count || 1))), 0),
    [booking.lines],
  );
  ...
  const revisedBase = useMemo(
    () =>
      rows.reduce(
        (sum, row) => sum + Math.max(0, numberValue(row.rate)) * Math.max(0, Math.trunc(numberValue(row.qty))),
        0,
      ),
    [rows],
  );
  ...
  const chargeUnitLabel = adjustmentChargeUnitLabel("PACKAGE");
    ...
    return (
      <>
        <div className="adj-add-row-actions">
          ...
          <div>
            <button type="button" onClick={() => setRows((current) => [...current, newRow("ADULT")])}>
              + Adult
            </button>
            <button type="button" onClick={() => setRows((current) => [...current, newRow("CHILD")])}>
              + Child
            </button>
            <button type="button" onClick={() => setRows((current) => [...current, newRow("INFANT")])}>
              + Infant
            </button>
          </div>
        </div>
                ...
                <tr key={row.rowId}>
                  <td>
                    <select
                      ...
                      onChange={(e) => updateRow(row.rowId, { passengerType: e.target.value as PackagePassengerType })}
                    >
                  ...
                  <td>
                    <b>{money(Math.max(0, numberValue(row.rate)) * Math.max(0, Math.trunc(numberValue(row.qty))))}</b>
                  </td>
                  <td>
          ...
          <tbody>
            {booking.lines.map((line) => {
              ...
              return (
                <tr key={line.id}>
                  <td>
                    <span className={`passenger-chip ${line.passenger_type.toLowerCase()}`}>{line.passenger_type}</span>{" "}
                  ...
                  <td>
                    <b>{money(qty * Number(line.rate_per_person || 0))}</b>
                  </td>
                </tr>
              );
            })}
