...
import {
  ...
  saveHotelCancellation,
  saveHotelCorrectionOrAmendment,
  type HotelAdjustmentRecord,
  type HotelAdjustmentType,
} from "./HotelAdjustmentDb";
...
const amendmentCategories = bookingLifecycleConfigs.HOTEL.amendmentTypes;
const roomOptions: Array<{ value: HotelRoomType; label: string }> = [
...
function lineTotalPkr(row: RowState) {
  const roe = numberValue(row.roe);
  if (roe <= 0) return 0;
  return (
    Math.max(0, numberValue(row.rateSar)) *
    Math.max(1, Math.trunc(numberValue(row.nights))) *
    Math.max(1, Math.trunc(numberValue(row.qty))) *
    roe
  );
}

function rowFromBooking(line: HotelBooking["lines"][number]): RowState {
  return {
    ...
    nights: String(Math.max(1, Number(line.nights || 1))),
    roomType: line.room_type,
    qty: String(Math.max(1, Number(line.quantity || 1))),
    rateSar: String(Number(line.rate_per_night_sar || 0)),
...
function adjustmentLabel(type: HotelAdjustmentType) {
...
export default function HotelBookingAdjustment({
  companyId,
  booking,
  userId = "",
  canEdit = true,
  ...
  const [history, setHistory] = useState<HotelAdjustmentRecord[]>([]);
  ...
  async function loadHistory() {
    try {
      setHistory(await getHotelAdjustmentHistory(companyId, booking.id));
      setError("");
    } catch (e) {
  ...
  const cancelledValue = useMemo(() => {
    ...
    return booking.lines.reduce((sum, line) => {
      const available = Math.max(1, Math.trunc(Number(line.quantity || 1)));
  ...
  function chooseType(type: HotelAdjustmentType) {
  ...
  async function save() {
    ...
    try {
      if (adjustmentType === "CORRECTION" || adjustmentType === "AMENDMENT") {
        const result = await saveHotelCorrectionOrAmendment(
          companyId,
          booking.id,
          {
            adjustmentType,
            ...
            lines: rows.map((row) => ({
              ...
              nights: Math.max(1, Math.trunc(numberValue(row.nights))),
              roomType: row.roomType,
              ratePerNightSar: numberValue(row.rateSar),
              quantity: Math.max(0, Math.trunc(numberValue(row.qty))),
        ...
        await onSaved?.(
          `${adjustmentLabel(adjustmentType)} saved for ${booking.ub_number}. ${accountNoun} adjustment: ${signedMoney(result.delta)}. Current Hotel value: ${money(result.effectiveTotal)}.`,
        );
      } else {
        const result = await saveHotelCancellation(
          companyId,
          booking.id,
          {
            adjustmentType,
        ...
        await onSaved?.(
          `${adjustmentLabel(adjustmentType)} saved for ${booking.ub_number}. Account credit: ${money(result.accountCredit)}. Current Hotel value: ${money(result.effectiveTotal)}.`,
        );
      }
      onClose();
    } catch (e) {
  ...
  function renderSelection() {
    const choices: Array<{ type: HotelAdjustmentType; title: string; text: string; badge: string }> = [
      {
        type: "CORRECTION",
        title: "Correction",
      ...
      },
      {
        ...
        text: "Cancel selected Hotel stays / room quantities and calculate cancellation charges and account credit.",
        badge: "SELECT ITEMS",
      },
      {
        type: "FULL_CANCELLATION",
        title: "Full Cancellation",
        text: "Cancel the complete Hotel booking while retaining any applicable cancellation charge.",
        badge: "FULL BOOKING",
      },
    ];
    return (
          ...
          <table className="adj-lines-table">
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.rowId}>
                  <td>
                    <input value={row.city} onChange={(e) => updateRow(row.rowId, { city: e.target.value })} />
                  </td>
                  <td>
                    <input
                      value={row.hotelName}
                  ...
                  <td>
                    <input
                      ...
                      step="1"
                      value={row.nights}
                      onChange={(e) => updateRow(row.rowId, { nights: e.target.value })}
                    />
                  </td>
                  <td>
                    <select
            ...
            {booking.lines.map((line) => {
              ...
              return (
                <tr key={line.id}>
                  ...
                  <td>
                    {line.check_in} → {line.check_out} · {line.nights}N
                  </td>
                  <td>{available}</td>
                  <td>
                    {adjustmentType === "FULL_CANCELLATION" ? (
  ...
  function renderHistory() {
    ...
    return (
      <div className="adj-history-view">
        ...
        {!history.length && (
          <div className="adj-empty-history">No booking adjustments yet. This is still the original Hotel booking.</div>
        )}
      </div>
    );
  }
