export const PRODUCT_NAME = "Travel Hisab";
export const PRODUCT_TAGLINE = "Travel Accounting and Booking Management";
export const COMPANY_NAME = "SMC Softwares";
export const PRODUCT_INITIALS = "TH";
export const PRODUCT_WINDOW_TITLE = PRODUCT_NAME;
export const PRODUCT_BYLINE = `${PRODUCT_NAME} by ${COMPANY_NAME}`;

export const PRODUCT_HIGHLIGHTS = [
  "Full Package, Hotel, Ticket, Visa, Transport & Misc bookings",
  "Party ledgers, payments and service-wise statements",
  "Amendments with revision history on every booking",
  "Desktop app with secure cloud sync for your team",
  "Company login with role-based user access",
] as const;
