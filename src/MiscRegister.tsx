...
const config = bookingLifecycleConfigs.MISC;
...
function bookingSummary(booking: MiscBooking) {
  return booking.lines.map((line) => `${line.service_name} · ${line.pax_count} pax`).join(" | ") || "Misc booking";
}

export default function MiscRegister({
  companyId,
  transactionType,
  userId = "",
  canEdit = true,
  ...
  const [bookings, setBookings] = useState<MiscBooking[]>([]);
  const [summaries, setSummaries] = useState<Record<string, MiscAdjustmentSummary>>({});
  ...
  async function load(nextSearch = search) {
    try {
      const [raw, nextSummaries] = await Promise.all([
        getMiscBookings(companyId, nextSearch),
  ...
  async function voidBooking(booking: MiscBooking) {
    ...
    setError("");
    try {
      await voidMiscBooking(companyId, booking.id, userId);
      await load();
      await onChanged?.();
  ...
  async function doDelete(booking: MiscBooking) {
    if (
      !window.confirm(
  ...
  async function openBooking(booking: MiscBooking, lifecycle: BookingLifecycleStatus) {
    ...
    <>
      <section className="booking-entry-screen package14-page package14-register-page lifecycle-register-page">
        <div className="booking-screen-toolbar package14-toolbar">
          ...
          <span className="booking-foundation-badge active-engine">MISC REGISTER</span>
        </div>
        <div className="package14-register-title">
          <div>
            <span className="eyebrow blue">MISC BOOKING REGISTER</span>
            <h2>{config.label} Booking Register</h2>
            <p>
              Open Booking, Booking Adjustment, History and Void Booking stay in the same Actions column. Misc
              adjustments are stored in dedicated Misc revision history.
            </p>
          </div>
        ...
        {!visible.length ? (
          <div className="empty-state compact-empty">
            <div className="empty-icon">MSC</div>
                          ...
                          {booking.lines.length ? (
                            booking.lines.map((line) => (
                              <div key={line.id}>
                                ...
                                <small>
                                  {line.pax_count} pax · {money(line.line_total_pkr)}
                                </small>
                              </div>
                            ))
                          ) : (
        ...
        >
          <section className="adj-shell lifecycle-preview" onMouseDown={(e) => e.stopPropagation()}>
            <div className="adj-toolbar">
              <div>
                <span className="eyebrow blue">OPEN MISC BOOKING</span>
            ...
            <section className="adj-section">
              <div className="adj-section-title">
                <span>02</span>
                <div>
                  <b>CURRENT MISC COMMERCIAL ROWS</b>
                  ...
                  <tbody>
                    {previewBooking.lines.map((line) => (
                      <tr key={line.id}>
                        ...
                        <td>{money(line.rate_per_person)}</td>
