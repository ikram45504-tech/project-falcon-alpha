  ...
  return (
    <main className="workspace">
        ...
        <div className="header-actions">
          <div className="minimal-user-row">
            <div className="minimal-user-text">
              ...
              <small>{session ? ROLE_LABELS[session.role] : ""} · {company?.company_code}</small>
            </div>
      ...
      <nav className="workspace-nav main-workspace-nav">
        ...
        {can("view_parties") && (
          <NavLink to="/parties" className={({ isActive }) => isActive || window.location.pathname.startsWith("/parties/") ? "active" : ""}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '10px' }}>
        ...
        {can("view_bookings") && (
          ...
          >
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '10px' }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17.8 19.2 16 11l3.5-3.5C21 6 21.5 4 21 3c-1-.5-3 0-4.5 1.5L13 8 4.8 6.2c-.5-.1-.9.2-1.1.6L2.5 9l8.1 4.5-3.6 3.6-3.4-.6c-.4-.1-.9.2-1.1.6l-.8 2 5.1 1.4 1.4 5.1 2-.8c.4-.2.7-.7.6-1.1l-.6-3.4 3.6-3.6 4.5 8.1c.4.7 1.3.8 2 .1.4-.4.8-1 .6-1.5z"></path></svg>
              Bookings
            </span>
          </NavLink>
        )}
        ...
        <NavLink to="/settings" className={({ isActive }) => isActive || window.location.pathname.startsWith("/settings") ? "active security-nav-button" : "security-nav-button"}>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: '10px' }}>
      ...
      <div className="layout-content-wrapper">
        ...
        <Route path="/parties/ledger/:id" element={
          company ? <LedgerRoute companyId={company.id} parties={parties} onChanged={loadFinancialTotals} setStatementPartyId={setStatementPartyId} /> : null
        } />
        ...
        <Route path="/bookings" element={
          company && session && can("view_bookings") ? (
            <BookingsModule
              ...
              onChanged={async () => { await loadParties(); await loadFinancialTotals(); }}
            />
          ) : <Navigate to="/" />
        } />
...
function LedgerRoute({ companyId, parties, onChanged, setStatementPartyId }: { companyId: string, parties: Party[], onChanged: () => void, setStatementPartyId: (id: string) => void }) {
  const { id } = useParams();
  const navigate = useNavigate();
  ...
  if (!session || !company) {
    return (
      <Routes>
        ...
        <Route path="/login" element={<LoginScreen accountCreatedNotice={accountCreatedNotice} setAccountCreatedNotice={setAccountCreatedNotice} />} />
