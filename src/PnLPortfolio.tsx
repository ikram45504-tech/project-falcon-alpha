...
export default function PnLPortfolio({ companyId, onBack }: Props) {
  ...
  const ubProfits = useMemo(() => {
    const ubMap = new Map<string, { ub: string; service: string; sale: number; purchase: number; parties: Set<string> }>();
    ...
    for (const [partyId, rows] of ledgers.entries()) {
      ...
      for (const row of rows) {
        ...
        const current = ubMap.get(key) || { ub: row.ref_no, service: row.service_type, sale: 0, purchase: 0, parties: new Set() };
    ...
    return Array.from(ubMap.values())
      ...
      .filter(item => searchUbTab ? (item.partyNames.toLowerCase().includes(searchUbTab.toLowerCase()) || item.ub.toLowerCase().includes(searchUbTab.toLowerCase())) : true)
  ...
  return <section className="booking-entry-screen bf-page bf-ub-control">
    <div className="booking-screen-toolbar">
      <button className="booking-back-button" onClick={onBack}>← Back to Dashboard</button>
    ...
    <div className="dashboard-subnav" style={{ marginBottom: 24, borderBottom: "1px solid var(--border)", paddingBottom: 16 }}>
      <button className={activeTab === "UB" ? "active" : ""} onClick={() => setActiveTab("UB")}>UB-Level Profitability</button>
      <button className={activeTab === "PARTY" ? "active" : ""} onClick={() => setActiveTab("PARTY")}>Party-Level Profitability</button>
      <button className={activeTab === "SERVICE" ? "active" : ""} onClick={() => setActiveTab("SERVICE")}>Service-Level Profitability</button>
    </div>
    ...
    {activeTab === "UB" && (
      <section className="bf-card">
        <div className="bf-section-head">
          <div>
            <span>01</span>
            <div><b>UMBRELLA BOOKING MARGINS (CROSS-SERVICE)</b><small>Aggregated Sale minus Purchase for every UB</small></div>
          </div>
          <div className="search-box" style={{ width: 300, alignSelf: "center", marginBottom: 0 }}>
            <span>🔍</span>
            <input type="text" placeholder="Search by Party or UB..." value={searchUbTab} onChange={e => setSearchUbTab(e.target.value)} />
          </div>
        </div>
        <div className="bf-table-wrap">
          <table className="bf-table">
            ...
            <tbody>
              {ubProfits.length === 0 ? <tr><td colSpan={8} className="bf-empty-cell">No UBs found.</td></tr> : ubProfits.map((ub, i) => (
                <tr key={ub.key}>
                  <td style={{ color: "#778595", fontSize: 11, fontWeight: 800 }}>{i + 1}</td>
                  <td><b>{ub.ub}</b></td>
                  <td><span className="party-name">{ub.partyNames}</span></td>
                  <td>{ub.service}</td>
                  <td className="bf-money" style={{ textAlign: "right" }}>{money(ub.sale)}</td>
                  <td className="bf-money" style={{ textAlign: "right" }}>{money(ub.purchase)}</td>
                  <td className="bf-money" style={{ textAlign: "right", color: ub.margin >= 0 ? "var(--green)" : "var(--red)", fontWeight: 600 }}>{money(ub.margin)}</td>
                  <td style={{ textAlign: "right", color: ub.margin >= 0 ? "var(--green)" : "var(--red)" }}>{ub.marginPercent.toFixed(1)}%</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    {activeTab === "PARTY" && (
      <section className="bf-card">
        <div className="bf-section-head">
          <div>
            <span>02</span>
            <div><b>PARTY PROFITABILITY</b><small>Aggregated profit per Customer / Agent</small></div>
          </div>
          <div className="search-box" style={{ width: 300, alignSelf: "center", marginBottom: 0 }}>
            <span>🔍</span>
            <input type="text" placeholder="Search party..." value={searchPartyTab} onChange={e => setSearchPartyTab(e.target.value)} />
          </div>
        </div>
        <div className="bf-table-wrap">
          <table className="bf-table">
            ...
            <tbody>
              {partyProfits.length === 0 ? <tr><td colSpan={5} className="bf-empty-cell">No parties found with transactions.</td></tr> : partyProfits.map((p, i) => (
                <tr key={p.partyName}>
                  <td style={{ color: "#778595", fontSize: 11, fontWeight: 800 }}>{i + 1}</td>
                  <td><b>{p.partyName}</b> <small>({p.accountType})</small></td>
                  <td className="bf-money" style={{ textAlign: "right" }}>{money(p.sale)}</td>
                  <td className="bf-money" style={{ textAlign: "right" }}>{money(p.purchase)}</td>
                  <td className="bf-money" style={{ textAlign: "right", color: p.margin >= 0 ? "var(--green)" : "var(--red)", fontWeight: 600 }}>{money(p.margin)}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    {activeTab === "SERVICE" && (
      <section className="bf-card">
        <div className="bf-section-head">
          <div>
            <span>03</span>
            <div><b>SERVICE PROFITABILITY</b><small>Aggregated profit per Service Type</small></div>
          </div>
        </div>
        <div className="bf-table-wrap">
          <table className="bf-table">
            ...
            <tbody>
              {serviceProfits.length === 0 ? <tr><td colSpan={4} className="bf-empty-cell">No services found.</td></tr> : serviceProfits.map(s => (
                <tr key={s.service}>
                  <td><b>{s.service}</b></td>
                  <td className="bf-money" style={{ textAlign: "right" }}>{money(s.sale)}</td>
                  <td className="bf-money" style={{ textAlign: "right" }}>{money(s.purchase)}</td>
                  <td className="bf-money" style={{ textAlign: "right", color: s.margin >= 0 ? "var(--green)" : "var(--red)", fontWeight: 600 }}>{money(s.margin)}</td>
                </tr>
              ))}
            </tbody>
          </table>
