...
export default function AppearanceScreen() {
  ...
  return (
    <section className="panel" style={{ position: "relative", padding: "40px" }}>
      ...
      <div style={{ position: "absolute", top: "40px", right: "40px", display: "flex", background: "var(--bg-app)", border: "1px solid var(--border-light)", borderRadius: "999px", padding: "4px", gap: "2px" }}>
        {([
          ...
          <button
            ...
            style={{
              ...
              border: "none", padding: "6px 14px", borderRadius: "999px", cursor: "pointer", fontWeight: 600, fontSize: "13px", transition: "all 0.2s", whiteSpace: "nowrap"
            }}
          >
            {mode === t.id && "✅ "}{t.icon} {t.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ maxWidth: "600px" }}>
        ...
        <p style={{ fontSize: "16px", color: "var(--text-muted)" }}>Completely transform the structural architecture of your software.</p>
      </div>
...
function LayoutCard({ title, desc, layoutId, currentLayout, onSelect }: { title: string, desc: string, layoutId: LayoutType, currentLayout: LayoutType, onSelect: (l: LayoutType) => void }) {
  ...
  return (
    ...
    >
      <h4 style={{ margin: "0 0 8px 0", color: "var(--text-main)", fontSize: "16px" }}>{isActive ? "✅ " : ""}{title}</h4>
