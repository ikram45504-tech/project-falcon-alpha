...
import { getMiscBookings, updateMiscBooking, voidMiscBooking, type MiscBooking } from "./miscDb";
...
import {
  getUniversalBookingAdjustmentSummaryMap,
  type UniversalAdjustmentSummary,
...
function inputValue(row: UniversalAdjustmentRow, key: string) {
...
function rowsFor(service: SupportedService, booking: RawBooking): UniversalAdjustmentRow[] {
    ...
    return b.lines.map((line) => {
      ...
      return {
        ...
        values: {
          ...
          visaRateSar: String(line.visa_rate_sar || 0),
          qty: String(line.pax_count || 1),
          roe: Number(line.roe || 0) > 0 ? String(line.roe) : "",
  ...
  if (service === "TRANSPORT") {
    ...
    return b.lines.map((line) => ({
      ...
      values: {
        ...
        vehicleCount: String(line.vehicle_count || 0),
        rateSar: String(line.rate_sar || 0),
        paxCount: String(line.pax_count || 0),
        roe: Number(line.roe || 0) > 0 ? String(line.roe) : "",
      },
    }));
  }
  ...
  return b.lines.map((line) => ({
    ...
    values: {
      ...
      qty: String(line.pax_count || 1),
      rate: String(line.rate_per_person || 0),
...
function calculateLine(service: SupportedService, row: UniversalAdjustmentRow) {
  ...
  if (service === "TRANSPORT") {
    ...
    return inputValue(row, "transportType") === "SHARING_BUS"
      ? rate * whole(inputValue(row, "paxCount")) * roe
      : rate * whole(inputValue(row, "vehicleCount")) * roe;
  }
...
function quantityFor(service: SupportedService, row: UniversalAdjustmentRow) {
  ...
  return inputValue(row, "transportType") === "SHARING_BUS"
    ...
    : Math.max(1, whole(inputValue(row, "vehicleCount") || "1"));
}

function withQuantity(
  service: SupportedService,
  row: UniversalAdjustmentRow,
  quantity: number,
): UniversalAdjustmentRow {
...
function describeRow(service: SupportedService, row: UniversalAdjustmentRow, index: number) {
  ...
  if (service === "TRANSPORT")
    return `${inputValue(row, "fromLocation") || "From"} → ${inputValue(row, "toLocation") || "To"} · ${inputValue(row, "customVehicleName") || inputValue(row, "vehicleType")}`;
...
function normalizeBookings(service: SupportedService, bookings: RawBooking[]): BookingRow[] {
  return bookings.map((booking) => {
      ...
      return {
        ...
        summary:
          b.lines
            .map(
              (line) =>
                `${line.from_location} → ${line.to_location} · ${line.custom_vehicle_name || line.vehicle_type}`,
            )
    ...
    return {
      ...b,
      transaction_type: b.transaction_type as BookingTransactionType,
      summary: b.lines.map((line) => `${line.service_name} · ${line.pax_count} pax`).join(" | ") || "Misc booking",
      totalSar: Number(b.total_sar || 0),
      raw: b,
    };
  });
...
export default function BookingLifecycleCenter({
  ...
  async function applyRows(booking: BookingRow, rows: UniversalAdjustmentRow[]) {
    if (service === "TICKET") {
      ...
      await updateTicketCommercialBooking(
        companyId,
        b.id,
        {
    ...
    if (service === "VISA") {
      ...
      await updateVisaBooking(
        ...
        b.id,
        {
          ...
          passports: b.passports.map((item) => ({
      ...
      await updateTransportBooking(
        ...
        b.id,
        {
          ...
          lines: rows.map((row) => {
            ...
            const vehicleType =
              transportType === "SHARING_BUS"
                ? "SHARING_BUS"
                : (inputValue(row, "vehicleType") as TransportVehicleType);
            return {
              ...
              vehicleCount:
                transportType === "PRIVATE_VEHICLE" ? Math.max(1, whole(inputValue(row, "vehicleCount"))) : 0,
    ...
    await updateMiscBooking(
      ...
      b.id,
      {
        ...
        ubNumber: b.ub_number,
        lines: rows.map((row) => ({
          serviceName: inputValue(row, "serviceName").trim(),
          paxCount: Math.max(1, whole(inputValue(row, "qty"))),
          ratePerPerson: Math.max(0, num(inputValue(row, "rate"))),
          roe: num(inputValue(row, "roe")) > 0 ? num(inputValue(row, "roe")) : null,
        })),
      },
      userId,
    ...
    <>
      <section className="booking-entry-screen package14-page package14-register-page lifecycle-register-page">
        <div className="booking-screen-toolbar package14-toolbar">
          ...
          <span className="booking-foundation-badge active-engine">{service} REGISTER</span>
        </div>
        <div className="package14-register-title">
          <div>
            <span className="eyebrow blue">{service} BOOKING REGISTER</span>
            <h2>{config.label} Booking Register</h2>
            <p>
              Open Booking, Booking Adjustment, History and Void Booking stay in the same Actions column for every
              booking service.
            </p>
          </div>
        ...
        {!visible.length ? (
          <div className="empty-state compact-empty">
            <div className="empty-icon">{service.slice(0, 3)}</div>
                ...
                {visible.map((booking) => {
                  ...
                  return (
                    <tr key={booking.id} className={booking.status === "VOID" ? "void-row" : ""}>
                      ...
                      <td>{cancelled ? "—" : booking.totalSar > 0 ? sar(booking.totalSar) : "—"}</td>
                      <td className="amount">
        ...
        >
          <section className="adj-shell lifecycle-preview" onMouseDown={(e) => e.stopPropagation()}>
            <div className="adj-toolbar">
              <div>
                <span className="eyebrow blue">OPEN {service} BOOKING</span>
            ...
            <section className="adj-section">
              <div className="adj-section-title">
                <span>02</span>
                <div>
                  <b>CURRENT {service} COMMERCIAL ROWS</b>
              ...
              <div className="adj-lines-table-wrap">
                <table className="adj-lines-table">
                  ...
                  <tbody>
                    {rowsFor(service, previewBooking.raw).map((row) => (
                      <tr key={row.id}>
                        {columnsFor(service).map((column) => (
                          <td key={column.key}>{inputValue(row, column.key) || "—"}</td>
                        ))}
                        <td>
                          <b>{money(calculateLine(service, row))}</b>
                        </td>
                      </tr>
                    ))}
                  </tbody>
