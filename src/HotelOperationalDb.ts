...
async function ensureSchema() {
  if (!schemaPromise) {
    schemaPromise = (async () => {
      ...
      await database.execute(`CREATE INDEX IF NOT EXISTS idx_hotel_operational_guests ON hotel_operational_guests(company_id,booking_id,sort_order)`);
    })().catch((error) => {
      schemaPromise = null;
      throw error;
    });
...
export async function saveHotelGuestRefs(companyId: string, bookingId: string, guestNames: string[], userId = "") {
  ...
  await ensureSchema();
  const database = await db();
  await database.execute(`DELETE FROM hotel_commercial_guest_refs WHERE company_id=$1 AND booking_id=$2`, [companyId, bookingId]);
...
export async function getHotelOperationalDetails(companyId: string, bookingId: string): Promise<HotelOperationalDetails> {
  await ensureSchema();
  const database = await db();
  ...
  const reservations = await database.select<Array<{ hotel_sort_order: number; confirmation_voucher: string; meal_plan: string; reservation_status: HotelReservationStatus }>>(
  ...
  const guests = await database.select<Array<{ id: string; given_name: string; surname: string; passport_number: string; hotel_sort_order: number; room_allocation: string; sort_order: number }>>(
  ...
  const meta = await database.select<Array<{ customer_contact: string; special_requests: string; checkin_instructions: string; notes: string }>>(
...
export async function saveHotelOperationalDetails(companyId: string, bookingId: string, input: SaveHotelOperationalInput, userId = "") {
  await requireEdit(companyId, userId);
  await ensureSchema();
  const database = await db();
  const now = new Date().toISOString();
  await database.execute(`DELETE FROM hotel_operational_reservations WHERE company_id=$1 AND booking_id=$2`, [companyId, bookingId]);
  await database.execute(`DELETE FROM hotel_operational_guests WHERE company_id=$1 AND booking_id=$2`, [companyId, bookingId]);
  for (const item of input.reservations) {
    await database.execute(
      `INSERT INTO hotel_operational_reservations (company_id,booking_id,hotel_sort_order,confirmation_voucher,meal_plan,reservation_status)
       VALUES ($1,$2,$3,$4,$5,$6)`,
      [companyId, bookingId, item.hotelSortOrder, item.confirmationVoucher.trim(), item.mealPlan.trim(), item.reservationStatus]
    );
  }
  for (const [index, guest] of input.guests.entries()) {
    await database.execute(
      ...
      [crypto.randomUUID(), companyId, bookingId, guest.givenName.trim(), guest.surname.trim(), guest.passportNumber.trim().toUpperCase(), guest.hotelSortOrder, guest.roomAllocation.trim(), index]
    );
  }
  await database.execute(
    ...
    [bookingId, companyId, input.customerContact.trim(), input.specialRequests.trim(), input.checkinInstructions.trim(), input.notes.trim(), now]
  );
  await audit(companyId, userId, bookingId, "Hotel reservation, voucher and rooming details updated without changing hotel totals.");
}
